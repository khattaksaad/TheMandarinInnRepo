...
namespace HotelManager
{
    public partial class fAdvanceBooking : Form
    {
        ...
        public fAdvanceBooking(string userName)
        {
        ...
        {
            txbFullName.Text = txbPhoneNumber.Text = cbRoomType.Text = cbSex.Text = String.Empty;
        ...
        private void btnBookRoom_Click(object sender, EventArgs e)
        {
            ...
            {
                if (txbFullName.Text != String.Empty && txbPhoneNumber.Text != String.Empty && txtBxCNIC.Text != string.Empty)
                {
                    if (!IsIdCardExists(txtBxCNIC.Text))
                    {
                        InsertCustomer(txtBxCNIC.Text, txbFullName.Text, 1, dpkDateOfBirth.Value,string.Empty, txbPhoneNumber.Text, string.Empty, string.Empty, cbSex.Text, string.Empty);
                    }
                    ...
                    foreach (var item in roomBooking4GridBindingSource.List)
                    {
                        ...
                        InsertAdvanceBooking(CustomerDAO.Instance.GetInfoByIdCard(txtBxCNIC.Text).Id, room.RoomTypeId, bookingType:0,dpkDateCheckIn.Value, dpkDateCheckOut.Value, DateTime.Now, 0 , this.userName);
                    }
