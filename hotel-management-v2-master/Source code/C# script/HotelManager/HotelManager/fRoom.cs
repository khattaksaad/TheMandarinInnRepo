...
namespace HotelManager
{
    public partial class fRoom : Form
    {
        ...
        {
            DialogResult result = MessageBox.Show( "Do you want to update?", "Question", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
        ...
        private void UpdateRoom()
        {
            ...
            {
                MessageBox.Show( "Room Id cannot be empty\n", "Stop", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            else
            if (!fCustomer.CheckFillInText(new Control[] { txbNameRoom, comboBoxStatusRoom, comboBoxRoomType }))
            {
                MessageBox.Show( "Please provide all details, first", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show( "Room already exists", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
                        ...
                        if (check)
                        {
                            ...
                            MessageBox.Show( "Cannot save it right now\n", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    }
                }
                catch
                {
        ...
        private void ComboBoxRoomType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (((DataTable)comboBoxRoomType.DataSource).Rows[index]["Price"].ToString().Contains("."))
                return;
            txbPrice.Text = ((int)((DataTable)comboBoxRoomType.DataSource).Rows[index]["Price"]).ToString();
        }
        #endregion
        ...
        private void dataGridViewRoom_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            {
                switch (Myrow.Cells[4].Value)
                {
                    case "Available":
                        Myrow.DefaultCellStyle.BackColor = Color.LightGreen;
                        break;
                    case "Booked":
                        Myrow.DefaultCellStyle.BackColor = Color.Coral;
                        break;
                    case "Not open for booking":
