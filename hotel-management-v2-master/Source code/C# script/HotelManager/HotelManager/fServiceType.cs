...
namespace HotelManager
{
    public partial class fServiceType : Form
    {
        ...
        {
            DialogResult result = MessageBox.Show("Do you want to update this type of service?", "Question", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
        ...
        private void ToolStripLabel1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (check)
                    ...
                    MessageBox.Show("Cannot export, Error occurred", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch
            {
        ...
        {
            if(comboboxID.Text == string.Empty)
                MessageBox.Show("This type of service does not yet exist", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            else
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (serviceTypeNow.Equals(serviceTypePre))
                        MessageBox.Show("You have not changed the data", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    else
                    {
                        bool check = ServiceTypeDAO.Instance.UpdateServiceType(serviceTypeNow);
                        if (check)
                        {
                            MessageBox.Show("Updated successfully", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            if (btnCancel.Visible == false)
                            {
                        ...
                        {
                            MessageBox.Show("This type of service does not exist yet.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
                catch
                {
                    MessageBox.Show("Service type error already exists", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
